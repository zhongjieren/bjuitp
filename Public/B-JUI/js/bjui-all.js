...
+function ($) {
    ...
    Bjuiajax.prototype.ajaxform = function(options) {
        ...
        var _submitFn = function() {
            var op = {loadingmask:that.options.loadingmask, type:$form.attr('method'), url:$form.attr('action'), callback:successFn, error:$.proxy(that.ajaxerror, that)}
    ...
    Bjuiajax.prototype.dosearch = function(options) {
        ...
        var search = function() {
            if ($target && $target.length) {
                ...
                that.reloadlayout({target:$target[0], type:$element.attr('method') || 'POST', url:options.url, data:data, loadingmask:options.loadingmask})
            } else {
                if (that.tools.getTarget() == Bjuiajax.NAVTAB) {
                    $target = $.CurrentNavtab
                    form    = that.tools.getPagerForm($target, op)
